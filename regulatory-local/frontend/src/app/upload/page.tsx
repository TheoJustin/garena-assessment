...
function uploadPdf(
  ...
  return new Promise<IngestionResponse>((resolve, reject) => {
    ...
    xhr.onload = () => {
      ...
      const errorResponse = parseXhrPayload<{ detail?: string; error?: string }>(
        xhr,
      );
...
export default function UploadPage() {
  ...
  const providerSummary = workflowStatus?.provider.reachable
    ? `${providerLabel} is reachable with ${workflowStatus.provider.chat_model}.`
    : workflowStatus?.provider.error ??
      'The provider is still being checked.';
      ...
      <div className="mx-auto max-w-5xl space-y-6">
          ...
          <div className="flex flex-col gap-5">
            <div className="max-w-3xl">
              ...
              <h1 className="mt-4 text-3xl font-semibold tracking-tight text-zinc-50 md:text-4xl">
                Upload, index, and move straight into regulatory Q&amp;A
              </h1>
        ...
        <Card className="border-zinc-800 bg-zinc-950 text-zinc-100 shadow-2xl">
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                <Button
                  ...
                  className="h-10 rounded-full border-zinc-700 bg-zinc-950 px-4 text-zinc-300 hover:bg-zinc-900 hover:text-zinc-100"
                >
                  {isRefreshing ? (
                ...
                <Link href="/rag-chat">
                  <Button className="h-10 rounded-full bg-zinc-100 px-4 text-zinc-900 hover:bg-zinc-200">
                    Open Chat
          ...
          <CardContent className="space-y-6 pt-6">
            <div className="grid gap-3 md:grid-cols-3">
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/65 p-4">
                ...
                <p className="mt-3 text-3xl font-semibold tracking-tight text-zinc-50">
                  {workflowStatus?.indexed_documents ?? 0}
                </p>
                <p className="mt-2 text-sm leading-6 text-zinc-500">
                  {workflowStatus?.indexed_documents
                    ? 'Documents are already available for retrieval.'
                    : 'Upload one PDF or seed the bundled set to start the local RAG flow.'}
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/65 p-4">
                ...
                <p className="mt-3 text-base font-semibold tracking-tight text-zinc-50">
                  {workflowStatus?.provider.chat_model ?? 'Checking model...'}
                </p>
                <p className="mt-2 text-sm leading-6 text-zinc-500">
                  {providerSummary}
                </p>
            ...
            {workflowError && (
              <div className="flex items-start gap-3 rounded-2xl border border-red-900/60 bg-red-950/20 px-4 py-3 text-sm text-red-100">
