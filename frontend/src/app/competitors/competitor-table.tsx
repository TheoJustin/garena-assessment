'use client';

import { useState } from 'react';
import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from '@/components/ui/table';
import { Card, CardContent } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { Database, FileText, Search, X } from 'lucide-react';

// Define the shape of our data
interface Competitor {
  id: number;
  competitor_name: string;
  feature_name: string;
  price: string | null;
  advantages: string | null;
  disadvantages: string | null;
  pdf_name: string | null;
}

export default function CompetitorTable({
  initialData,
}: {
  initialData: Competitor[];
}) {
  const [searchQuery, setSearchQuery] = useState('');

  // Live filter the data based on the search query
  const filteredData = initialData.filter((row) => {
    const query = searchQuery.toLowerCase();
    const matchName =
      row.competitor_name?.toLowerCase().includes(query) || false;
    const matchPdf = row.pdf_name?.toLowerCase().includes(query) || false;
    const matchFeature =
      row.feature_name?.toLowerCase().includes(query) || false;

    return matchName || matchPdf || matchFeature;
  });

  return (
    <div className="min-h-screen bg-zinc-950 p-8 text-zinc-100">
      <div className="mx-auto max-w-6xl">
        {/* Header Section */}
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-3xl font-semibold tracking-tight">
              Competitor Analysis
            </h1>
            <p className="mt-1 text-zinc-400">
              Extracted feature comparisons and competitor analysis.
            </p>
          </div>

          <div className="flex h-9 items-center gap-2 rounded-md border border-zinc-800 bg-zinc-900/50 px-4 text-sm text-zinc-400">
            <Database className="h-4 w-4" />
            <span>{filteredData.length} Records</span>
          </div>
        </div>

        {/* Live Search Bar Section */}
        <div className="mb-8 relative w-full max-w-3xl">
          <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-zinc-500" />
          <Input
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search by competitor, feature, or PDF name..."
            className="h-12 w-full rounded-lg border-zinc-800 bg-zinc-900/50 pl-12 pr-12 text-base text-zinc-200 placeholder:text-zinc-500 focus-visible:ring-zinc-700"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery('')}
              className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-zinc-300 transition-colors"
            >
              <X className="h-5 w-5" />
            </button>
          )}
        </div>

        {/* Table Section */}
        <Card className="border-zinc-800 bg-zinc-900/30 shadow-none">
          <CardContent className="p-0">
            <div className="w-full rounded-md">
              <Table className="w-full table-fixed">
                <TableHeader className="border-b border-zinc-800 bg-zinc-900/50">
                  <TableRow className="border-zinc-800 hover:bg-transparent">
                    <TableHead className="w-[20%] py-4 pl-6 font-medium text-zinc-300">
                      Competitor
                    </TableHead>
                    <TableHead className="w-[20%] py-4 font-medium text-zinc-300">
                      Feature
                    </TableHead>
                    <TableHead className="w-[10%] py-4 font-medium text-zinc-300">
                      Price
                    </TableHead>
                    <TableHead className="w-[25%] py-4 font-medium text-zinc-300">
                      Advantages
                    </TableHead>
                    <TableHead className="w-[25%] py-4 pr-6 font-medium text-zinc-300">
                      Disadvantages
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredData.length === 0 ? (
                    <TableRow className="border-zinc-800">
                      <TableCell
                        colSpan={5}
                        className="h-32 text-center text-zinc-500"
                      >
                        {searchQuery
                          ? `No results found for "${searchQuery}"`
                          : 'No competitor data found. Upload a PDF first.'}
                      </TableCell>
                    </TableRow>
                  ) : (
                    filteredData.map((row) => (
                      <TableRow
                        key={row.id}
                        className="border-zinc-800 transition-colors hover:bg-zinc-900/50"
                      >
                        <TableCell className="break-words py-4 pl-6 align-top whitespace-normal">
                          <div className="flex flex-col gap-1">
                            <span className="font-medium text-zinc-200">
                              {row.competitor_name}
                            </span>
                            {row.pdf_name && (
                              <span className="flex items-center text-xs font-normal text-zinc-500">
                                <FileText className="mr-1.5 h-3 w-3 shrink-0" />
                                <span className="truncate" title={row.pdf_name}>
                                  {row.pdf_name}
                                </span>
                              </span>
                            )}
                          </div>
                        </TableCell>
                        <TableCell className="break-words py-4 align-top text-zinc-300 whitespace-normal">
                          {row.feature_name}
                        </TableCell>
                        <TableCell className="py-4 align-top">
                          {row.price ? (
                            <span className="inline-flex items-center rounded-md bg-zinc-800/80 px-2 py-1 text-xs font-medium text-zinc-300 ring-1 ring-inset ring-zinc-700/50 break-words whitespace-normal">
                              {row.price}
                            </span>
                          ) : (
                            <span className="text-sm text-zinc-600">—</span>
                          )}
                        </TableCell>
                        <TableCell className="break-words py-4 pr-4 align-top text-sm leading-relaxed text-zinc-400 whitespace-normal">
                          {row.advantages || (
                            <span className="text-zinc-600">—</span>
                          )}
                        </TableCell>
                        <TableCell className="break-words py-4 pr-6 align-top text-sm leading-relaxed text-zinc-400 whitespace-normal">
                          {row.disadvantages || (
                            <span className="text-zinc-600">—</span>
                          )}
                        </TableCell>
                      </TableRow>
                    ))
                  )}
                </TableBody>
              </Table>
            </div>
          </CardContent>
        </Card>
      </div>
    </div>
  );
}
